...
class OrderController {
    ...
    private ?CommerceSettingsPort $settingsPort = null;
    ...
    private function settings(): CommerceSettingsPort {
        return $this->settingsPort ??= CommerceCollaborationPortsFactory::settings();
    }
    ...
    private function respondWithOrderPage(?string $userId = null): void
    {
        $pageSize = BoundedPageRequest::pageSize($_GET, 100, 100);
        $cursor = BoundedPageRequest::cursor($_GET, 'pedidos');
        $page = $userId === null
            ? $this->orderRepository->getPageResult(['limit' => $pageSize, 'cursor' => $cursor])
            : $this->orderRepository->getByUserIdPage($userId, ['limit' => $pageSize, 'cursor' => $cursor]);
        $nextCursor = $page['nextPosition'] !== null
            ? CreatedAtIdCursor::encode($page['nextPosition'])
            : null;

        header('X-Orders-Page-Size: ' . $pageSize);
        header('X-Orders-Has-More: ' . ($nextCursor !== null ? 'true' : 'false'));
        if ($nextCursor !== null) {
        ...
        $responseBytes = OrderListSummary::assertResponseBudget($page['items'], $meta);
        header('X-Orders-Projection: ' . OrderListSummary::CONTRACT);
        header('X-Orders-Response-Bytes: ' . $responseBytes);
        header('X-Orders-Max-Response-Bytes: ' . OrderListSummary::MAX_RESPONSE_BYTES);
        Response::json($page['items'], 200, $meta);
    }
    ...
    public function index() {
        ...
        try {
            ...
            if ($isAdmin) {
                $this->respondWithOrderPage();
            } else {
                if (empty($user['sub'])) {
                    Response::error('No autorizado', 403, 'AUTH_FORBIDDEN');
                    return;
                }
                $this->respondWithOrderPage((string)$user['sub']);
            }
        } catch (\InvalidArgumentException $e) {
            Response::error($e->getMessage(), 400, 'ORDERS_PAGINATION_INVALID');
        } catch (\Exception $e) {
    ...
    public function myOrders() {
        ...
        try {
            ...
            $this->respondWithOrderPage((string)$user['sub']); // 'sub' is user id in JWT
        } catch (\InvalidArgumentException $e) {
            Response::error($e->getMessage(), 400, 'ORDERS_PAGINATION_INVALID');
        } catch (\Exception $e) {
            Response::error($e->getMessage(), 500, 'ORDERS_LIST_FAILED');
    ...
    private function isStoreSalesEnabled() {
        $enabledRaw = $this->settings()->get('store_sales_enabled');
        $messageRaw = $this->settings()->get('store_sales_message');
