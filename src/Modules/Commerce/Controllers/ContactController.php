...
{
    private CommerceContactPort $contact;

    public function __construct()
    {
        $this->contact = CommerceCollaborationPortsFactory::contact();
    }
    ...
    public function store(): void
    {
        ...
        $record = $this->contact->createMessage([
            'name' => $name,
            'email' => $email,
            'phone' => $phone,
            'subject' => $subject,
    ...
    private function sendNotification(array $record, string $replyToEmail, string $replyToName): bool
    {
        ...
        return $this->contact->sendMail($destination, $subject, $message, $replyToEmail, $replyToName);
    }
    ...
    private function sendCustomerConfirmation(array $record, string $recipientEmail, string $recipientName): bool
    {
        ...
        return $this->contact->sendMail($recipientEmail, $subject, $message, $contactDestination, $tenantName);
    }
    ...
    private function isRateLimited(string $email, ?string $ipAddress): bool
    {
        ...
        return $this->contact->countRecentByEmail($email) >= $emailLimit
            || $this->contact->countRecentByIp($ipAddress) >= $ipLimit;
    }
