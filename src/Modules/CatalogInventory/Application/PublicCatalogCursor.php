...
final class PublicCatalogCursor
{
    ...
    public static function decode(?string $token): ?array
    {
        ...
        if (!is_array($payload) || ($payload['v'] ?? null) !== self::VERSION) {
            throw new \InvalidArgumentException('Versión de cursor de catálogo no soportada.');
        }

        $createdAt = trim((string)($payload['createdAt'] ?? ''));
        $id = trim((string)($payload['id'] ?? ''));
        if ($createdAt === '' || $id === '' || strlen($id) > 255 || strlen($createdAt) > 64) {
            throw new \InvalidArgumentException('Cursor de catálogo incompleto.');
        }
        try {
            new \DateTimeImmutable($createdAt);
        } catch (\Exception) {
            throw new \InvalidArgumentException('Fecha de cursor de catálogo inválida.');
        }
