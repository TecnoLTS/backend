...
class ProductController {
    ...
    private function enablePublicCatalogCache(): void {
    ...
    private function publicCatalogPageSize(): int {
        $raw = $_GET['page_size'] ?? ($_GET['limit'] ?? 48);
        if (is_array($raw) || preg_match('/^[1-9][0-9]*$/', trim((string)$raw)) !== 1) {
            throw new \InvalidArgumentException('page_size debe ser un entero entre 1 y 100.');
        }

        $pageSize = (int)$raw;
        if ($pageSize < 1 || $pageSize > 100) {
            throw new \InvalidArgumentException('page_size debe estar entre 1 y 100.');
        }

        return $pageSize;
    }

    private function adminCatalogPageSize(): int {
        $raw = $_GET['page_size'] ?? ($_GET['limit'] ?? 100);
        if (is_array($raw) || preg_match('/^[1-9][0-9]*$/', trim((string)$raw)) !== 1) {
            throw new \InvalidArgumentException('page_size debe ser un entero entre 1 y 100.');
        }
    ...
    private function publicCatalogCursor(): ?array {
        $raw = $_GET['cursor'] ?? null;
        if (is_array($raw)) {
            throw new \InvalidArgumentException('Cursor de catálogo inválido.');
        }
        return PublicCatalogCursor::decode($raw === null ? null : (string)$raw);
    }

    private function publicCatalogFilters(): array {
        return PublicCatalogFilters::fromQuery($_GET);
    }

    private function publicCatalogNextLink(string $nextCursor, int $pageSize): string {
        $query = $_GET;
        unset($query['limit']);
        $query['page_size'] = $pageSize;
        $query['cursor'] = $nextCursor;
        $path = parse_url((string)($_SERVER['REQUEST_URI'] ?? '/api/products'), PHP_URL_PATH) ?: '/api/products';
    ...
    private function respondWithAdminCatalogPage(): void {
        ...
        $pageSize = $this->adminCatalogPageSize();
        $page = $this->productRepository->getAdminPage(
            $pageSize,
            $this->publicCatalogCursor(),
            [
                'includeProcurement' => $includeProcurement,
            ]
        );
        $nextCursor = $page['nextPosition'] !== null
            ? PublicCatalogCursor::encode($page['nextPosition'])
            : null;

        header('X-Catalog-Page-Size: ' . $pageSize);
        header('X-Catalog-Has-More: ' . ($nextCursor !== null ? 'true' : 'false'));
        if ($nextCursor !== null) {
            header('X-Next-Cursor: ' . $nextCursor);
            header('Link: <' . $this->publicCatalogNextLink($nextCursor, $pageSize) . '>; rel="next"');
        }
        Response::json($page['items'], 200, [
            'pagination' => [
                'pageSize' => $pageSize,
    ...
    private function normalizeTaxSettings(array &$data, ?array $currentProduct = null): void {
        ...
        $hasTaxRate = array_key_exists('taxRate', $data)
            || array_key_exists('taxRate', $attributes)
            || array_key_exists('tax_rate', $attributes)
            || array_key_exists('taxRate', $currentAttributes)
            || array_key_exists('tax_rate', $currentAttributes);
        $rawTaxRate = $data['taxRate']
            ?? $attributes['taxRate']
            ?? $attributes['tax_rate']
            ?? $currentAttributes['taxRate']
            ?? $currentAttributes['tax_rate']
            ?? null;
        if ($attributes['taxExempt'] === 'true') {
            $attributes['taxRate'] = '0';
        } elseif ($hasTaxRate) {
            try {
                $taxRate = EcuadorSriVatCatalog::assertSupportedRate($rawTaxRate);
            } catch (\InvalidArgumentException) {
                Response::error(
                    'Tarifa IVA del producto no soportada por el catálogo SRI.',
                    400,
                    'PRODUCT_TAX_RATE_UNSUPPORTED'
                );
                exit;
            }
            $attributes['taxRate'] = rtrim(rtrim(number_format($taxRate, 2, '.', ''), '0'), '.');
        }
        unset(
            $attributes['tax_exempt'],
            $attributes['tax_rate'],
            $attributes['taxTreatment'],
            $attributes['tax_treatment'],
            $data['taxExempt'],
            $data['taxRate']
        );
    ...
    public function index() {
        try {
            ...
            if ($includeUnpublished) {
                $this->respondWithAdminCatalogPage();
                return;
            }

            $pageSize = $this->publicCatalogPageSize();
            $page = $this->productRepository->getPublicPage(
                $pageSize,
                $this->publicCatalogCursor(),
                $this->publicCatalogFilters()
            );
            $products = PublicProductProjection::many($page['items']);
            $nextCursor = $page['nextPosition'] !== null
                ? PublicCatalogCursor::encode($page['nextPosition'])
                : null;

            $this->enablePublicCatalogCache();
            header('X-Catalog-Page-Size: ' . $pageSize);
            header('X-Catalog-Has-More: ' . ($nextCursor !== null ? 'true' : 'false'));
            if ($nextCursor !== null) {
                header('X-Next-Cursor: ' . $nextCursor);
            ...
            Response::error($e->getMessage(), 400, 'CATALOG_PAGINATION_INVALID');
        } catch (\Exception $e) {
    ...
    public function adminIndex() {
        ...
        try {
            ...
            Response::error($e->getMessage(), 400, 'CATALOG_ADMIN_PAGINATION_INVALID');
        } catch (\Exception $e) {
    ...
    public function show($id) {
        try {
            ...
            if (!$includeUnpublished) {
                $product = PublicProductProjection::one($product);
                $this->enablePublicCatalogCache();
            }
