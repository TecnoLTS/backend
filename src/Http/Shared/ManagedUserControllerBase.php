...
abstract class ManagedUserControllerBase {
    ...
    public function ecommerceUsers() {
        ...
        try {
            ...
            $pageSize = BoundedPageRequest::pageSize($query, 100, 100);
            $cursor = BoundedPageRequest::cursor($query, 'clientes ecommerce');
            $legacyPageMode = array_key_exists('page', $query) && $cursor === null;
            $page = $legacyPageMode ? BoundedPageRequest::legacyPage($query, 50) : 1;
            $users = $this->customerRepository->getPage([
                'search' => $search,
                'role' => $role,
                'limit' => $pageSize + 1,
                'offset' => $legacyPageMode ? ($page - 1) * $pageSize : 0,
                'cursor' => $cursor,
            ]);
            ...
            if ($hasMore && $users !== []) {
                ...
                $nextCursor = CreatedAtIdCursor::encode([
            ...
            header('X-Customers-Page-Size: ' . $pageSize);
            header('X-Customers-Has-More: ' . ($hasMore ? 'true' : 'false'));
            if ($nextCursor !== null) {
            ...
            Response::json($users, 200, [
                'pagination' => [
                    'page' => $legacyPageMode ? $page : null,
                    'pageSize' => $pageSize,
                    'hasMore' => $hasMore,
                    'nextCursor' => $nextCursor,
                    'nextPage' => $legacyPageMode && $hasMore ? $page + 1 : null,
                    'mode' => $legacyPageMode ? 'legacy-page' : 'cursor',
                ],
            ]);
        } catch (\InvalidArgumentException $e) {
            Response::error($e->getMessage(), 400, 'ECOMMERCE_USERS_PAGINATION_INVALID');
        } catch (\Exception $e) {
    ...
    public function accessAudit(): void {
        Auth::requireAdmin();
        Response::noStore();
        $pageSize = max(1, min(100, (int)($_GET['pageSize'] ?? $_GET['limit'] ?? 50)));
